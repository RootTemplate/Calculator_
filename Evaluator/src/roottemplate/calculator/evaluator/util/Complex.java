 ...
 * - Some methods are commented out
 * - Added log10(), log(Complex), floor(), root(Complex) methods
 */
...
public class Complex {
    ...
    /** Complex {@code of}-th root. (Not checked, testing required.)
     */
    public final Complex root(Complex of) {
        if(of.re == 2 && of.im == 0)
            return sqrt();
        return pow(new Complex(1, 0).div(of));
    }
    ...
    /** Complex floor (also known as [x]). (Not checked, testing required.)
     */
    public final Complex floor() {
        double a = re;
        double b = im;
        if (b == 0) {
            return set(Math.floor(a), 0);
        }
        return set(Math.floor(re), Math.floor(im)); // In mod() Math.rint is used, maybe we should use it here also?
    }
    ...
    /** Complex logarithm base-10. (Not checked, testing required.)
     */
    public final Complex log10() {
        if(im == 0)
            return set(Math.log10(re), 0);

        return set(0.5D * Math.log(abs2()) / MoreMath.LOG10, Math.atan2(im, re) / MoreMath.LOG10);
    }

    /** Complex logarithm, any base. (Not checked, testing required.)
     */
    public final Complex log(Complex base) {
        if(im == 0 && base.im == 0)
            return set(MoreMath.log(re, base.re), 0);

        double tRe = base.re, tIm = base.im;
        log().div(base.log());
        base.set(tRe, tIm); // those values were changed after calling base.log()
        return this;
    }
