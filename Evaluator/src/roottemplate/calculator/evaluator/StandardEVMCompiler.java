...
import roottemplate.calculator.evaluator.util.StackEVMCompiler;
...
public class StandardEVMCompiler extends StackEVMCompiler {
    @Override
    public StandardExpression compile(ListStruct exprElems) throws EvaluatorException {
        Kit kit = new Kit();
        ExpressionElement result = compileExprElems(exprElems, kit);
        return new StandardExpression(
                kit.insns.toArray(new Instruction[kit.insns.size()]),
                result, kit.indexes.getBufferSize()
        );
    }
    ...
    protected ExpressionElement compileOperator(Operator op, ExpressionElement[] ns, Object kit_) throws EvaluatorException {
        ...
        if(op.getUses() == Operator.Uses.TWO_NUMBERS && op.isCommutative() && refCount == 1 && !hasModifiableNumbers) {
            ...
            int n1Index;
            if(lastInstr.op.equals(op) && (n1Index = lastInstr.findUnmodifiableNumber()) != -1) {
        ...
        if(hasModifiable) {
            ...
            return new Instruction(op, -1, ns).execute(null);
        }
    }
